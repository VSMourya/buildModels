#' Fit a Linear Elastic Net Regression Model and Evaluate R-squared
#'
#' This function fits an Elastic Net model using the `glmnet` package. It preprocesses data,
#' optionally selects the top K features, fits the model, evaluates the R-squared on the testing dataset,
...
#'
#' @return A list containing the fitted model, R-squared on test data, and the predictors used.
#' @import glmnet
#' @examples
#' data(iris)
#' iris$Species <- as.numeric(iris$Species)
#' result <- fit_linear_elastic_net(data = iris, predictors = c("Sepal.Length", "Sepal.Width"), response = "Species")
#' print(result)
#' @export
...
fit_linear_elastic_net <- function(data, predictors, response, lambda = 1, top_k = NULL) {
  ...
  saveRDS(elastic_model, "elastic_net_model.rds")

  x_test <- model.matrix(reformulate(predictors, response), data_test)[, -1]
  predictions_test <- predict(elastic_model, newx = x_test, s = lambda)
  r_squared_test <- 1 - sum((data_test[[response]] - predictions_test)^2) / sum((data_test[[response]] - mean(data_test[[response]]))^2)

  cat("R-squared on test data: ", r_squared_test, "\n")
  cat("Elastic Net model is saved in the present directory under the name 'elastic_net_model.rds'\n")

  return(list(model = elastic_model, r_squared_test = r_squared_test, predictors = predictors))
}
